...
typedef ap_fixed<WBITS+FRACBITS,INTBITS+FRACBITS,AP_RND_ZERO,AP_SAT,NBITS> fixedtype2;
typedef ap_int<WBITS> fixedtype3;
typedef ap_int<INTBITS> fixedtypeU;
typedef ap_int<FRACBITS> fixedtypeL;

fixedtype ONE(1),RES(pow(BASE,-FRACBITS)),onlyFRAC(ONE-RES);
void howDoesBitsWork();

// copied directly from VivadoLogSystem
using namespace std;

string sign(bool b);
int convertToInt(A_LogNum L);
double convertToDouble(A_LogNum L);
...
void howDoesBitsWork() {
    double x[TABLEROWS];
    fixedtype X[TABLEROWS];
    fixedtype MIN_VAL = fixedtype(-1*pow(BASE,INTBITS-1));
    double base(MIN_VAL),res(RES);
    ...
    int offset = pow(2,WBITS-1);

    for (int i = 0; i < TABLEROWS; ++i) {
        std::cout << "\nX[" << i << "] is : " << X[i];
        std::cout << " and x[" << i << "] was : " << x[i] << std::endl;
        std::cout << "Bit representation of X: " << std::bitset<INTBITS>(X[i]) << "_" <<
                std::bitset<FRACBITS>(((fixedtype2)(X[i] & onlyFRAC))<<FRACBITS) << " equals ";
        std::string extendedIntIndex =  std::bitset<INTBITS>(X[i]).to_string() + std::bitset<FRACBITS>(((fixedtype2)(X[i] & onlyFRAC))<<FRACBITS).to_string();
        std::cout << extendedIntIndex;

        fixedtype3 D_extended = fixedtype3(X[i].range(WBITS-1,0));
        int val = D_extended; // not working

        int extendedInt = (((fixedtype2)(X[i])) << FRACBITS) + offset;
        std:: cout << " = ? " << extendedInt << " (the array index)" << std::endl;

        // so to go from D --> index --> delta(D)
        /*
         * 1. extend D to <WBITS>.<FRACBITS>
         * 2. logical shift left D by FRACBITS to get:          // can we reduce the amount of shifting we have to do at all?
         *          <D>.0000
         * 3. cast result to int, and add offset = 2^(WBITS-1)
         */

    }

    fixedtype min_val = fixedtype(-1*pow(BASE,INTBITS-1));
    int min_val_int(min_val);

    std::cout << "\n\nmin_val " << min_val << "\nmin_val_int " << min_val_int << std::endl;
    std:: cout << "Bit representation of min_val: " << std::bitset<WBITS>(min_val) << std::endl;
...
void runTests() {
    ...
    for (int i = 0; i < TESTCASES; ++i) {
        experimentalSums[i] = convertToDouble(A_LogNum::addReals(
    ...
    for (int i = 0; i < TESTCASES; ++i) {
        experimentalProducts[i] = convertToDouble(A_LogNum::multiplyReals(
...
void runMSETest() {
    ...
    for (int i = 0; i < TESTCASES; ++i) {
        ...
        A_LogNum R1(toLogNum(r1)), R2(toLogNum(r2)),R3(toLogNum(r3));
        ...
        double calcMUL = convertToDouble(A_LogNum::multiplyReals(R2,R3));
        double refMUL = r2*r3;
        double MULdiff = refMUL-calcMUL;
        double acc_ref_diff = (convertToDouble(A_LogNum::addReals(R2,R3))-(r2+r3));
...
*/
        ...
        // log forms
        A_LogNum PR1(toLogNum(pr1)), PR2(toLogNum(pr2)),PR3(toLogNum(pr3));
        double d_deltaplus = abs(PR2.getLogval()-PR3.getLogval());
        A_LogNum MR1(toLogNum(mr1)), MR2(toLogNum(mr2)),MR3(toLogNum(mr3));
        ...
        //Dplus tests
        A_LogNum p_mac_result = PR1;
        p_mac_result.MAC(PR2,PR3);

        // MSE on multiplication, addition and overall MAC
        double p_calcMUL = convertToDouble(A_LogNum::multiplyReals(PR2,PR3));
        ...
        double p_acc_ref_diff = (convertToDouble(A_LogNum::addReals(PR2,PR3))-(pr2+pr3));
        ...
        // Dminus tests
        A_LogNum m_mac_result = MR1;
        m_mac_result.MAC(MR2,MR3);

        // see where errors came from
        double m_calcMUL = convertToDouble(A_LogNum::multiplyReals(MR2,MR3));
        double m_refMUL = mr2*mr3;

        double m_MULdiff = m_refMUL-m_calcMUL;
        M_MSE_MUL += m_MULdiff*m_MULdiff;

        double m_acc_ref_diff = (convertToDouble(A_LogNum::addReals(MR2,MR3))-(mr2+mr3));
...
void runMSE_Delta_Test_Procedure() {
    ...
    for (int i = 0; i < TESTCASES; ++i) {
        ...
        A_LogNum A(toLogNum(a)),Bplus(toLogNum(bplus)),Bminus(toLogNum(bminus));
        A_LogNum plusResult = A_LogNum::addReals(A,Bplus);
        A_LogNum minusResult = A_LogNum::addReals(A,Bminus);
